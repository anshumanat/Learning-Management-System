...
export default function Instructor() {
    ...
    useEffect(() => {
      ;(async () => {
        setLoading(true)
        if (!token) {
          // If no token, force logout or redirect (optional)
          setLoading(false)
          return;
        }
    ...
    const totalAmount = (Array.isArray(instructorData) ? instructorData : []).reduce(
      (acc, curr) => acc + (curr.totalAmountGenerated || 0),
      0
    )
    const totalStudents = (Array.isArray(instructorData) ? instructorData : []).reduce(
